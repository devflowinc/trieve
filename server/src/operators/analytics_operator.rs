...
pub async fn get_search_metrics_query(
    ...
    let mut query_string = String::from(
        "SELECT 
            ...
            round(100 * countIf(toFloat64OrNull(query_rating) = 1) / total_queries, 2) as percent_thumbs_up,
            round(100 * countIf(toFloat64OrNull(query_rating) = 0) / total_queries, 2) as percent_thumbs_down
        FROM search_queries
        WHERE dataset_id = ?            
         ",
    );
...
pub async fn get_rag_query_ratings_query(
    ...
    let mut query_string = String::from(
        "SELECT 
            round(100 * countIf(toFloat64OrNull(query_rating) = 1) / count(*), 2) as percent_thumbs_up,
            round(100 * countIf(toFloat64OrNull(query_rating) = 0) / count(*), 2) as percent_thumbs_down
        FROM 
            rag_queries
        WHERE dataset_id = ?",
    );
