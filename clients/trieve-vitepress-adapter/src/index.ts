...
const extractMarkdownPaths = (docsFolderPath: string): string[] => {
  const mdPaths = [];
  const files = fs.readdirSync(docsFolderPath);
  for (const file of files) {
    const fullPath = join(docsFolderPath, file);
    if (
      fs.statSync(fullPath).isDirectory() &&
      !fullPath.includes('node_modules')
    ) {
      mdPaths.push(...extractMarkdownPaths(fullPath));
    } else if (file.endsWith('.mdx') || file.endsWith('.md')) {
      mdPaths.push(docsFolderPath + '/' + file);
    }
  }
    ...
    for (const path of paths) {
      ...
      for (const method of methods) {
        ...
        const endpoint = namespace
          ...
          : path;
        const pageLink = `${siteUrl}/${apiRefParent}/${operationId}`;
        const metadata = {
...
if (
  ...
  if (!trieveApiHost) {
    console.error('\tTRIEVE_API_HOST');
  }
  if (!trieveApiKey) {
    console.error('\tTRIEVE_API_KEY');
  }
  if (!trieveOrganizationId) {
    console.error('\tTRIEVE_ORGANIZATION_ID');
  }
  if (!trieveDatasetTrackingId) {
    console.error('\tTRIEVE_DATASET_TRACKING_ID');
  }
  process.exit(1);
}
...
program
  .option('-p, --path <path>', 'path to the Vitepress docs directory')
...
if (!folderPath) {
  console.error('Missing required --path option', options);
  program.help();
}
...
try {
  const paths = extractMarkdownPaths(folderPath);
  ...
  console.error(`Error reading markdown files from ${folderPath}`, err);
  process.exit(1);
}
