...
register(async ({ analytics, browser, init, settings }) => {
  ...
  let fingerprint = await browser.localStorage
    .getItem("trieve-fingerprint")
    .catch((_) => {
      return null;
    });
  ...
  analytics.subscribe("checkout_started", async (event) => {
    ...
    const lastMessage = JSON.parse(
      (await browser.localStorage.getItem("lastMessage").catch((_) => {
        return null;
      })) ?? "{}",
    );
  ...
  analytics.subscribe("product_added_to_cart", async (event) => {
    ...
    for (const id in lastMessage) {
      const storedItems = lastMessage[id];
      if (storedItems.some((item: string) => item === cart_item)) {
        requestId = id;
        break;
      }
    }
