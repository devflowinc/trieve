...
const SendIcon = () => {
  return (
    ...
    >
      <path d="m1 1.91.78-.41L15 7.449v.95L1.78 14.33 1 13.91 2.583 8 1 1.91ZM3.612 8.5 2.33 13.13 13.5 7.9 2.33 2.839l1.282 4.6L9 7.5v1H3.612Z"></path>
    </svg>
  );
};
      ...
      setSelectedSidebarFilters((prev) => {
        if (prev.length !== 0) {
          return prev.map((filter, index) => {
            ...
            if (
              sections[index].filterType === "range" &&
              sections[index].selectionType === "range"
            ) {
              const parameters = toolCall.parameters as {
                min_value: number;
                max_value: number;
              };
              const minValue = Math.max(
                parameters.min_value,
                sections[index].options[0].range?.min || 0,
              );
              const maxValue = Math.min(
                parameters.max_value,
                sections[index].options[0].range?.max || Infinity,
              );
              if (filter.section.key === sections[index].key) {
                return { ...filter, range: { min: minValue, max: maxValue } };
              }
            } else {
              const selectedTags = Object.entries(
                toolCall.parameters as Record<string, boolean>,
              )
